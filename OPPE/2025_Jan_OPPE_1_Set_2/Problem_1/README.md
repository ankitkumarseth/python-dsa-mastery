...
Write a function `is_both_divisible_by` that takes three integers `a`, `b` and `c` as input and checks if `a` and `b` numbers are divisible by `c`. Assume that `c` is non-zero.
