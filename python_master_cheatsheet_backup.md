...
## 9. Advanced Idioms (Extracted from Practice)
...
### `any()` and `all()` Short-Circuiting
Pass a **lazy generator expression** `(x for x in arr)` rather than a built list `[x for x in arr]` to `any()`. This ensures it stops evaluating exactly when it hits `True`, giving you optimal $O(N)$ early-exit performance without writing a manual loop.
```python
# Optimal, early-exit short circuit:
result = any(num % 3 == 0 for num in numbers_list)
```
...
### List Comprehension for Conditional Mapping
Instead of copying a list and manually iterating to mutate the copy index-by-index, you can map values dynamically in one line using a list comprehension with an inline `if/else`.
```python
# Instead of:
# result = items[:]
...
Use this when you want to keep every single item, but conditionally change its value.
```python
